...
class Trainer(Action):
    ...
    def setup_model(self):
        ...
        model = Sequential()
        model.add((LSTM(self.config["RNN-train"].getint("rnn_output_features")+64+128, input_shape=input_shape, return_sequences=True)))
        model.add(Dropout(self.config["RNN-train"].getfloat("dropout")))
        model.add(BatchNormalization())
        model.add((LSTM(self.config["RNN-train"].getint("rnn_output_features")+64, return_sequences=True)))
        model.add(Dropout(self.config["RNN-train"].getfloat("dropout")))
        model.add(BatchNormalization())
        model.add((LSTM(self.config["RNN-train"].getint("rnn_output_features"))))
        model.add(Dropout(self.config["RNN-train"].getfloat("dropout")))
        model.add(BatchNormalization())
